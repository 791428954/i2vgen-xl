...
class UNetSD_SHARED_T2VBase(nn.Module):
    def __init__(self,
            config=None,
            in_dim=4,
            dim=512,
            ...
            **kwargs):
        super(UNetSD_SHARED_T2VBase, self).__init__()
        ...
        self.conv_out_shared = nn.Sequential(
            nn.GroupNorm(32, out_dim),
            nn.SiLU(),
            nn.Conv2d(out_dim, self.out_dim * 2, 3, padding=1))
        nn.init.zeros_(self.conv_out_shared[-1].weight)
    ...
    def forward(self,
        ...
        **kwargs):
        ...
        x = self.conv_out_shared(x)
        # reshape back to (b c f h w)
        if double_frame_flag:
            x = rearrange(x, '(b f) (i c) h w -> b c (f i) h w', b = batch,i=2)
        else:
            x = reduce(x, '(b f) (c 2)  h w -> b c f h w', 'mean',b = batch)
        return x
